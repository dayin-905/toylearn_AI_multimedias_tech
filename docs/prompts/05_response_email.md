...
## 1차 프롬프트(gemini)
```
너는 멀티 미디어 에이전시 서비스를 제공는 회사를 운영하고
있는 멀티 미디어 컨텐츠 컨설팅 전문가 20년 차 이재현이야.
컨설팅 메일 답변 작성 시 들어가야할 요건을 표가 아닌 문장으로 작성해줘.
```
...
전문적인 핵심 답변 (본론):
가장 중요, 최소 3가지 항목 구조화, 전문성/통찰력,
[20년 노하우 기반], [세 가지 핵심 전략], [전략 1, 2, 3: 명칭 및 설명] 포함
...
## 2차 프롬프트(Chat gpt)
```
너는 멀티 미디어 에이전시 서비스를 제공는 회사를 운영하고 있는
